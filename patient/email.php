     ...
     <?php
        ...
        if(isset($_POST['email_send'])){
          if(isset($_POST['mail'])){
             ...
             $sql="SELECT Email FROM `patient` where Email='$md_email'";
              ...
              if(mysqli_num_rows($result) != 0){
                echo "document.getElementById('error').innerText='This MailId Already exists'";
                    ...
                    if (mail($to_email, $subject, $body, $headers)) {
                        echo "$('#verify').show(); $('#sendmail').hide(); console.log('Success');";
                    } else {
                        echo "document.getElementById('error').innerText='Wrong MailId';$('#error').show(); x.focus();console.log('Fail');";  
                    }
              }   
        }else {
      ...
      if(isset($_POST['email_verify'])){
          if(isset($_POST['code'])){
            if ($_SESSION['no'] == $_POST['code']) {
                $_SESSION['patient']['email']=$to_email;
